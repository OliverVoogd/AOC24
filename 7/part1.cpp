...
private:
	...
	bool can_make_true(int eq_idx) const;

	static bool can_make_true_aux(const long long &value, const vector<long long> &eq, int i);
...
bool Calibrations::can_make_true(int eq_idx) const
{
	return can_make_true_aux(test_values[eq_idx], equations[eq_idx], equations[eq_idx].size() - 1);
}

bool Calibrations::can_make_true_aux(const long long &value, const vector<long long> &eq, int i)
{
	...
	if (value % eq[i] == 0)
	{
		possible = can_make_true_aux(value / eq[i], eq, i - 1);
	}
